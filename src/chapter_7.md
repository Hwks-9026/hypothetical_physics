...
Another common 'Circuit' solving tactic is known as Kirchhoffs (pronounced Jirk-off<sup>1</sup>) laws. First we will discuss the junction rule. This says that the voltage at each branch of a junction must sum to zero: 
$$\sum V = 0$$. 
An equivalent statement is:
$$\sum V_{in} = \sum V_{out}$$

The next important law is the loop rule, which says that the net current in a closed loop must be 0, because you are ending up at the same point you are starting at. You will often see this written as:
$$\sum i_n = 0$$
This can be derived from our power equation via integration:
